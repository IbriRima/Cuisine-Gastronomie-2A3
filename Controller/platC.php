    ...
    class platC {
        ...
        public function updateplat($plat, $id) {
            try {
                $pdo = getConnexion();
                $query = $pdo->prepare(
                    'UPDATE plat SET Nom_plat = :Nom_plat, Type_plat = :Type_plat, Prix_plat = :Prix_plat, Nbr_Clri_plat = :Nbr_Clri_plat, Pds_Portion_plat = :Pds_Portion_plat  WHERE Id_plat = :id'
                );
                $query->execute([
                    'Nom_plat' => $plat->getNomplat(),
                    'Type_plat' => $plat->getTypeplat(),
